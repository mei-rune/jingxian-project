	...
	if(Propfind(arrProps, szRemoteLoc))
	{
		...
		if(inStream.IsOk())
		{
			...
			if(xmlPropfind.IsOk())
			{
				...
				while(pNode)
				{
					if(pNode->GetName().Find("multistatus") != -1)
					{
				...
				while(pNode)
				{
					...
					if(pNode->GetName().Find("response") != -1)
					{
...
bool wxCurlDAVTool::ParseResponseXml(wxCurlDAVFs& fsItem, wxXmlNode* pNode)
{
	if(pNode->GetName().Find("response") != -1)
	{
		...
		while(pChild)
		{
			...
			if(!bParsedPropstat)
			{
				if(pChild->GetName().Find("propstat") != -1)
				{
...
{
	if(pNode->GetName().Find("propstat") != -1)
	{
		wxXmlNode* pChild = pNode->GetChildren();
		...
		while(pChild)
		{
			if(pChild->GetName().Find("status") != -1)
			{
				wxXmlNode* pText = pChild->GetChildren();
...
bool wxCurlDAVTool::ParsePropsXml(wxCurlDAVFs& fsItem, wxXmlNode* pNode)
{
	if(pNode->GetName().Find("prop") != -1)
	{
		...
		while(pChild)
		{
			if(pChild->GetName().Find("creationdate") != -1)
			{
				wxXmlNode* pText = pChild->GetChildren();
				...
				while(pText)
				{
					if(pText->GetType() == wxXML_TEXT_NODE)
					{
						...
						fsItem.m_dtCreationDate.ParseFormat(szVal, "%Y-%m-%dT%H:%M:%S");
			...
			if(pChild->GetName().Find("getlastmodified") != -1)
			{
				wxXmlNode* pText = pChild->GetChildren();
			...
			if(pChild->GetName().Find("getcontentlength") != -1)
			{
				wxXmlNode* pText = pChild->GetChildren();
			...
			if(pChild->GetName().Find("getcontenttype") != -1)
			{
				wxXmlNode* pText = pChild->GetChildren();
